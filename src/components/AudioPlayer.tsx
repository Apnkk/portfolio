...
export const AudioPlayer = ({ isPlaying, onTogglePlay }: AudioPlayerProps) => {
  ...
  // Compute duration in seconds for accurate progress calculation
  ...
  // MUST depend on isExpanded so it runs as soon as the panel opens!
  useEffect(() => {
    if (!isExpanded) {
    ...
    const draw = () => {
      ...
      const waves = [
        { freq: 0.022, speed: 3.2, color: '#f2a33c', alpha: 0.95, width: 2.2, blur: 12 },
        { freq: 0.03, speed: -2.6, color: '#ff6b3d', alpha: 0.8, width: 1.8, blur: 8 },
        { freq: 0.016, speed: 4.0, color: '#ffd166', alpha: 0.6, width: 1.4, blur: 6 },
        { freq: 0.038, speed: -3.6, color: 'rgba(255, 245, 235, 0.5)', alpha: 0.45, width: 1.0, blur: 4 },
      ];

      // Dynamic amplitude scaling based on live audio energy
      const ampBase = isPlaying ? Math.max(9, (0.3 + energy * 0.8) * (height * 0.42)) : 5;
      ...
      waves.forEach((w) => {
        ...
        ctx.shadowBlur = isPlaying ? w.blur : 4;

        const step = 4;
        for (let x = 0; x <= width; x += step) {
          // Hanning window function: anchors wave softly at edges and lets center dance
          ...
          const yOffset =
            (Math.sin(x * w.freq + phase) * ampBase +
              Math.cos(x * w.freq * 1.5 - phase * 0.8) * (ampBase * 0.45)) *
            windowFunc;
      ...
      <AnimatePresence>
        {isExpanded && (
          <motion.div
            ...
            className="w-[330px] sm:w-[360px] p-4 rounded-3xl bg-[#0a0a0cf5] border border-white/[0.12] backdrop-blur-3xl shadow-[0_25px_60px_-15px_rgba(0,0,0,0.95)] flex flex-col gap-3 font-sans text-left"
          >
