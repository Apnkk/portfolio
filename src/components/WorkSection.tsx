...
const ProjectRowItem = ({ project, idx, onSelect }: ProjectRowItemProps) => {
  ...
  // Subtle 3D physical pitch as camera moves over the card
  ...
  return (
    <motion.article
      ...
      className="py-10 sm:py-14 group will-change-transform"
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 mt-8 items-start">
        {/* Visual Art Preview Card (3D Tilt & Parallax Video-like Framing) */}
        <motion.div
          style={{ rotateX }}
          onClick={() => onSelect(project)}
          className="lg:col-span-5 relative aspect-[16/10] rounded-xl overflow-hidden border border-[rgba(237,232,221,0.12)] cursor-pointer group-hover:border-[rgba(242,163,60,0.5)] transition-colors duration-500 shadow-2xl bg-black transform-gpu will-change-transform"
        >
          {project.image ? (
            ...
            <div
              className="absolute inset-0 p-5 flex flex-col justify-between font-mono text-[11px] text-[#ede8dd]/60 select-none"
              style={{ background: bg }}
            >
